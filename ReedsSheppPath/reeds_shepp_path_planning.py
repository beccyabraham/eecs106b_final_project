...
def calc_limited_paths(sx, sy, syaw, gx, gy, gyaw, maxc, step_size=0.1):
    q0 = [sx, sy, syaw]
    q1 = [gx, gy, gyaw]

    path_params = [(-0.1, 'S', 1), (-0.1, 'L', 1), (-0.1, 'R', 1), (0.1, 'S', 1), (0.1, 'L', 1), (0.1, 'R', 1)]
    paths = []

    for l, m, c in path_params:
        l = l * 10
        path = Path()
        x, y, yaw, directions = generate_local_course(abs(l * c), [l * c], [m], c, c * step_size)
        # convert global coordinate
        path.x = [math.cos(-q0[2]) * ix + math.sin(-q0[2])
                  * iy + q0[0] for (ix, iy) in zip(x, y)]
        path.y = [-math.sin(-q0[2]) * ix + math.cos(-q0[2])
    ...
    for l1, m1, c1 in path_params:
        for l2, m2, c2 in path_params:
            l = l * 10
            path = Path()
            x, y, yaw, directions = generate_local_course(abs(l1 * c1) + abs(l2 * c1), [l1 * c1, l2 * c1], [m1, m2], c1, c1 * step_size)
            # convert global coordinate
            path.x = [math.cos(-q0[2]) * ix + math.sin(-q0[2])
                      * iy + q0[0] for (ix, iy) in zip(x, y)]
            path.y = [-math.sin(-q0[2]) * ix + math.cos(-q0[2])
...
def limited_path_planning(sx, sy, syaw, gx, gy, gyaw, maxc, step_size=0.01):
    paths = calc_limited_paths(sx, sy, syaw, gx, gy, gyaw, maxc, step_size)
    if not paths:
    ...
    for path in paths:
        x_diff = abs(path.x[-1] - gx)
        y_diff = abs(path.y[-1] - gy)
        yaw_diff = abs(mod2pi(path.yaw[-1]) - mod2pi(gyaw))
        if yaw_diff > math.pi:
            yaw_diff = (2 * math.pi) - yaw_diff
        newD = math.sqrt((x_diff ** 2) + (y_diff ** 2))
        newD += yaw_diff * .1
        if newD < minD:
            minD = newD
            best_path = path
...
def reeds_shepp_path_planning(sx, sy, syaw,
                              gx, gy, gyaw, maxc, step_size=0.01):
...
def main():
    ...
    #px, py, pyaw, directions = generate_local_course(1.0, [-1.0], ['L'], 1.0, 0.1)
